...
import numpy as np
from Mut.mkinputs import PDBreader, structure, vector, Geometry, getPhiPsi, mkCSF15, mkTRR100
...
def read_inputs(filenames, preparation_config):
    """
    7pc + 19psp + 8ss + 3ss + 4pp + 1prob / trr100 + csf15
    """
    inputs_1ds = []
    inputs_2ds = []
    inputs_total_len = 0
    ...
    for filename in filenames:
        ...
        inputs_1d = np.concatenate((inputs_1d, inputs_dlpacker[:,13500:], inputs_dlpacker[:,:13500]),-1)  
        
        assert inputs_1d.shape == (length, 41 + 1 + 13500)
...
#=============================================================================    
def get_ensemble_ouput(name, rota_predictions, lddt_predictions, L):
...
def output_results(filename, x1_output, x2_output, x3_output, x4_output, rmsd_output, std_output, preparation_config):
    ...
    output_path = os.path.join(preparation_config["output_path"], filename+".mut")
